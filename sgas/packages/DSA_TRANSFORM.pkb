...
AS
    ...
    END;       
      ...
      BEGIN
        ...
        q.QUOTE_REFERENCE AS quote_ref_no,
        q.OBJECT_ID AS object_id,
        q.session_code AS session_code,
        q.stud_ref_no AS stud_ref_no
        FROM dsa_quote_pdf@WEB q
        where q.stud_ref_no = v_stud_ref_no
        and q.object_id IS NOT NULL
        and q.session_code = v_session_code;
          
      EXCEPTION
       WHEN OTHERS
       THEN
          error_boolean := error_boolean || TO_CHAR (SQLCODE);
      END;                                          
